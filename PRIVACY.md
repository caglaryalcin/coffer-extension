...
If you select **Keep unlocked**, Coffer keeps the minimum resume key material plus the account identifier, Coffer origin, vault identifier, and fixed expiry time for up to 12 hours in extension-only `storage.local`. This record is persisted in the browser profile so Coffer can restore the session after a browser or Manifest V3 background-context restart without storing your password for this purpose. Because the resume key material is written to disk, use this option only on a trusted device. The extension deletes the record when you click **Lock**, its fixed lifetime expires, or you change the Coffer URL.
